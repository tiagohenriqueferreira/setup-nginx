...
echo -e "\n${GREEN}Configurando Zsh...${NC}"
{
  ...
  echo "  fi"
  echo ""
  echo "  # Detectar domínio automaticamente pelo vhost do Nginx"
  echo "  local domain=\"\""
  echo "  for vhost in /etc/nginx/sites-available/*; do"
  echo "    if grep -q \"\$project_web\" \"\$vhost\" 2>/dev/null; then"
  echo "      domain=\$(basename \"\$vhost\")"
  echo "      break"
  echo "    fi"
  echo "  done"
  echo ""
  echo "  if [ -z \"\$domain\" ]; then"
  ...
  echo "  fi"
  echo ""
  echo "  echo \"Domínio detectado automaticamente: \$domain\""
  echo ""
  ...
  echo ""
  echo "  # Escapar pontos do domínio para regex"
  echo '  local domain_escaped=$(echo "$domain" | sed "s/\\./\\\\\\\\./g")'
  echo ""
  ...
  echo "  echo \"  ✓ trusted_host_patterns → \$domain\""
